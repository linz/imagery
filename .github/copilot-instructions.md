...
This repository contains STAC Collection metadata for New Zealand aerial imagery datasets. Each dataset's metadata lives under `stac/{region}/{survey}/{product}/{projection}/collection.json`. When reviewing changes to `collection.json` files, check the following rules.
...
- If the title contains `"Near-Infrared"` then the product directory name in the repository path (`stac/{region}/{survey}/{product}/` must be `rgbnir`.
